...
func TestSetTypesWithSchemaRef(t *testing.T) {
	...
	if newtype.String() != "object<apiVersion: string, kind: enum string<\"Pod\">, metadata: object<annotations: object[any: any], clusterName: string, creationTimestamp: string, deletionGracePeriodSeconds: number, deletionTimestamp: string, finalizers: array[string], generateName: string, generation: number, initializers: object<pending: array[object<name: string>], result: object<apiVersion: string, code: number, details: object<causes: array[object<field: string, message: string, reason: string>], group: string, kind: string, name: string, retryAfterSeconds: number, uid: string>, kind: enum string<\"Status\">, message: string, metadata: object<continue: string, resourceVersion: string, selfLink: string>, reason: string, status: string>>, labels: object[any: any], managedFields: array[object<apiVersion: string, fields: object[any: any], manager: string, operation: string, time: string>], name: string, namespace: string, ownerReferences: array[object<apiVersion: string, blockOwnerDeletion: boolean, controller: boolean, kind: string, name: string, uid: string>], resourceVersion: string, selfLink: string, uid: string>>" {
...
func TestEnumSchemas(t *testing.T) {
	...
	enumStrProps := []*types.StaticProperty{
		types.NewStaticProperty("server",
			types.NewEnum(types.S, []string{"one", "two", "three"}))}
	...
	enumNumProps := []*types.StaticProperty{
		types.NewStaticProperty("server",
			types.NewEnum(types.N, []string{"1", "2", "3"}))}
	...
	enumNullProps := []*types.StaticProperty{
		types.NewStaticProperty("server",
			types.NewEnum(types.NewNull(), []string{"null"}))}
	...
	enumBoolProps := []*types.StaticProperty{
		types.NewStaticProperty("server",
			types.NewEnum(types.B, []string{"true"}))}
